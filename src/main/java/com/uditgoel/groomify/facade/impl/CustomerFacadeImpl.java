...
import com.uditgoel.groomify.dao.repository.CustomerOrderHistoryRepository;
import com.uditgoel.groomify.dao.repository.CustomerOrderRepository;
...
import com.uditgoel.groomify.dto.customer.CustomerJobsInput;
	...
	private List<CustomerJobCard> createMultipleJobCardsNew(JobStatus jobStatus,
		...
		createCustomerOrderInput.getCreateNewServicesMap().entrySet().forEach(serviceDateNameEntry -> {
			...
			for (int i = 0; i < serviceDateNameEntry.getValue().size(); i++) {
				...
				com.uditgoel.groomify.dao.services.Service service = serviceRepository.findByName(serviceName)
