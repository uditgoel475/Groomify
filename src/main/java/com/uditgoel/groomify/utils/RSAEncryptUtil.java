...
public class RSAEncryptUtil {
	...
	private static PublicKey getPublicKey(String base64PublicKey) {
		...
		try {
			...
			logger.error(String.format("Invalid Public Key Algorithm used : %s", e.getMessage()));
		} catch (InvalidKeySpecException e) {
			logger.error(String.format("Invalid Public Key specification used : %s", e.getMessage()));
		}
		return publicKey;
	}
	...
	private static PrivateKey getPrivateKey(String base64PrivateKey) {
		...
		try {
			...
			logger.error(String.format("Invalid Private Key Algorithm used : %s", e.getMessage()));
		} catch (InvalidKeySpecException e) {
			logger.error(String.format("Invalid Private Key specification used : %s", e.getMessage()));
		}
		return privateKey;
	}
