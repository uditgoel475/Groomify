...
public interface ServiceRepository extends PagingAndSortingRepository<Service, Long> {
	...
	@Query("select case when count(s)> 0 then true else false end from Service s LEFT JOIN s.serviceGroup st where s.name = :name and s.hsn = :hsn and st.name = :stName")
	...
	@Query("select s from Service s LEFT JOIN FETCH s.serviceGroup st where st.name = :stName and s.isActive = TRUE order by s.name asc")
